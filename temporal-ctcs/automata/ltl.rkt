...
(define ltl-accepts? machine-accepts?)
(define ltl->machine
  (match-lambda
    [(ltl:P ?)
     (machine 
      (λ (x_0)
        (if (? x_0)
            ...
            machine-null)))]
    [(ltl:X f)
     (machine-accepting (λ (input) (ltl->machine f)))]
    [(ltl:not f)
     (machine-complement (ltl->machine f))]
    [(ltl:and phi psi)
     (machine-intersect (ltl->machine phi) (ltl->machine psi))]
    [(ltl:U phi psi)
     (ltl->machine
      (ltl:or psi (ltl:and phi (ltl:X (ltl:U phi psi)))))]))
...
(define-syntax-rule (test-ltl l f ans)
  (test (models l f) => ans
        (ltl-accepts? (ltl->machine f) l) => ans))
...
(test
 ...
 (local [(define only-odd?-once (ltl:always (ltl:-> (ltl:P odd?) (ltl:X (ltl:always (ltl:not (ltl:P odd?)))))))]
   (test
